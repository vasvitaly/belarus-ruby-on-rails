...
class Notifier < ActionMailer::Base
  ...
  def new_participant_for_meetup_for_admin(meetup, participant)
    ...
    mail :to => "info@paaspro.by", :subject => "#{meetup.topic} - новый участник мероприятия", :content_type => "text/plain"
  end

  def removed_participant_for_meetup_for_admin(meetup, participant)
    @meetup = meetup
    @participant = participant
    mail :to => "info@paaspro.by", :subject => "#{@meetup.topic} - удален участник мероприятия", :content_type => "text/plain"
  end
  ...
  def user_unsubscribed(user)
    @user = user
    mail :to => "info@paaspro.by", :subject => "Пользователь отписался от рассылок", :content_type => "text/plain"
  end
end
